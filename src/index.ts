...
const DATA_GO_KR_BASE_URL = 'https://apis.data.go.kr/1160100/service/GetStockSecuritiesInfoService/getStockPriceInfo';
...
app.post('/api/search', async (c) => {
  try {
    ...
    console.log('Received query:', normalizedQuery, 'Type:', typeof normalizedQuery);
    ...
    // 2024년 대한민국 공휴일 목록
    const holidays2024 = [
      '20240101', '20240209', '20240210', '20240211', '20240212', // 설날
      '20240301', // 삼일절
      '20240410', // 총선
      '20240505', // 어린이날
      '20240506', // 대체공휴일
    ...
    while (targetDate.getDay() === 0 || targetDate.getDay() === 6 || holidays2024.includes(dateStr)) {
    ...
    const url = `${DATA_GO_KR_BASE_URL}?serviceKey=${apiKey}&resultType=json&basDt=${basDt}&itmsNm=${encodeURIComponent(normalizedQuery)}`;
    ...
    console.log('공공데이터 API Response:', JSON.stringify(responseData, null, 2));
...
app.post('/api/chart', async (c) => {
  try {
    ...
    const url = `${DATA_GO_KR_BASE_URL}?serviceKey=${apiKey}&resultType=json&beginBasDt=${beginBasDt}&endBasDt=${endBasDt}&itmsNm=${encodeURIComponent(code)}&numOfRows=300`;
    ...
    const sortedData = dataArray
      .sort((a: any, b: any) => a.basDt.localeCompare(b.basDt));
...
전일대비: ${change > 0 ? '+' : ''}${change?.toLocaleString()}원 (${changeRate > 0 ? '+' : ''}${changeRate}%)
...
(향후 주가 전망을 단기/중기로 나누어 2-3문장으로 설명. 단, 이는 참고용이며 투자 판단은 본인의 책임임을 명시)`;
    ...
    const analysis = geminiData.candidates?.[0]?.content?.parts?.[0]?.text || 'AI 분석 결과를 가져올 수 없습니다.';
  ...
  try {
    ...
    const stocksWithReasons = await Promise.all(
      dataArray.map(async (item: any) => {
        ...
        if (geminiApiKey) {
          try {
            const prompt = `${item.itmsNm} 종목이 ${parseFloat(item.fltRt || 0).toFixed(2)}% 급등했습니다. 이 종목의 급등 이유를 한 문장(20자 이내)으로 간단히 설명해주세요. 구체적인 사업 분야나 최근 이슈 중심으로 작성해주세요.`;
            ...
            if (geminiResponse.ok) {
              ...
              if (aiReason) {
                reason = aiReason.slice(0, 50); // 최대 50자로 제한
              }
            }
          } catch (error) {
